...
bool Memory::SetFPGA()
{
	...
	if ((qwVersionMajor >= 4) && ((qwVersionMajor >= 5) || (qwVersionMinor >= 7)))
	{
		...
		strcpy_s(config.szDevice, "existing");
...
size_t Memory::GetBaseDaddy(std::string module_name, DWORD pId)
{
	std::wstring str(module_name.begin(), module_name.end());

	PVMMDLL_MAP_MODULEENTRY module_info;
	if (!VMMDLL_Map_GetModuleFromNameW(this->vHandle, pId, const_cast<LPWSTR>(str.c_str()), &module_info, VMMDLL_MODULE_FLAG_NORMAL))
	{
		LOG("[!] Couldn't find Base Address for %s\n", module_name.c_str());
		return 0;
	}
...
DWORD Memory::GetTslGamePID()
{
	...
	{
		uint64_t UWorld = Read<uint64_t>(GetBaseDaddy(process_name, pid) + Offset::UWorld, pid);
		if (UWorld > 0)
		{
			...
			GameData.PhysxInstancePtr = this->Read<uint64_t>(this->current_process.base_address + Offset::Physx);
			PROCESS_INITIALIZED = true;
...
void Memory::RefreshMem()
{
	VMMDLL_ConfigSet(this->vHandle, VMMDLL_OPT_REFRESH_FREQ_MEM, NULL);
}